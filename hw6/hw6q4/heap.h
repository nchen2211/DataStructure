...
// child(i,j) of parent(i) = d * i + 1, ... , d * i + d
  ...
  class MinHeap {
   ...
   private:
    ...
    int parent_index(int); // returns parent index
    int child_index(int,int); // returns child index
    void bubbleUp(int); // helper function for add
    void trickleDown(int,int); // helper function for remove
    int smallestChild(int); // returns smallestChild index then used in remove function
  };
