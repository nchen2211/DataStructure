...
LListInt LListInt::operator+(const LListInt& other) const {
  ...
  //other is the second list
   LListInt appendList(*this);

    Item* curr = other.head_;
    Item* append = appendList.tail_;

    append->next = curr; //conjoin both lists

    int loc = this->size();

    while (curr != NULL) {
      appendList.insert(loc, curr->val);
      curr = curr->next;
      loc++;
    }
